...
export class ArkQRCode {
  ...
  validateAddress() {
    ...
    if (this.address && !this.address.match(pattern)) {
  ...
  @Watch("vendorField")
  validateVendorField() {
    if (typeof this.vendorField !== "string") {
      throw new Error("vendorField: must be a UTF-8 encoded string");
    }

    if (decodeURIComponent(this.vendorField).length > 255) {
      throw new Error("vendorField: enter no more than 255 characters");
    }
  }
  ...
  @Watch("showLogo")
  validateShowLogo() {
    if (typeof this.showLogo !== "boolean") {
      throw new Error("show-logo: must be a boolean");
    }

    if (this.showLogo && this.size < 150) {
      throw new Error(
        "show-logo: to display the logo the size must be greater than 150"
      );
    }
  }
  ...
  @Method()
  async getDataURL(mime: string = "image/png") {
  ...
  @Method()
  async deserializeURI(uri: string) {
    ...
    scheme.vendorField = scheme.vendorField
      ? this.fullyDecodeURI(scheme.vendorField)
      : null;
  ...
  @Method()
  async fromObject(data: any): Promise<HTMLElement> {
  ...
  generateSchema(): string {
    return JSON.parse(
      JSON.stringify(`ark:${this.address}${this.formatParams()}`)
    );
  }
  ...
  drawLogo(qr: QRious): void {
    ...
    const ctx = this.element.shadowRoot
  ...
  formatParams(): string {
    let params = [];
    const vendorField = encodeURIComponent(
      this.fullyDecodeURI(this.vendorField)
    );
    ...
    if (this.vendorField) {
      params.push(`vendorField=${vendorField}`);
    }
  ...
  componentDidUpdate() {
    return this.generateQRCode(this.generateSchema());
  }
